...
internal var mouseOverColor : Color = Color.green;  // mouse over color for the cursor
...
function OnGUI () {
	...
	if (useText && !conversing){  //global toggle
	   ...
	   if (showText && !showActionMsg){ //local toggle
	      ...
	      if (showShortDesc) 
	      GUI.Label (Rect (Screen.width/2 - 250, Screen.height - 70, 500,35), shortDesc, customGUIStyle);
	   }
	}
...
function NewSettings(playerSettings: String[]) {
   var fpController = GameObject.Find("First Person Controller");
   //update the array and assign the individual vars
   fpController.GetComponent(CharacterMotor).movement.maxForwardSpeed = parseFloat(playerSettings[0]);
   fpController.GetComponent(FPAdventurerInputController).rotationSpeed = parseFloat(playerSettings[1]); 
   ...
   mouseOverColor = GetComponent(MenuManager).cursorColors[parseInt(playerSettings[8])];
}
