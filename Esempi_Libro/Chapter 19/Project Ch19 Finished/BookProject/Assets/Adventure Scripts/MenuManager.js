...
function Start () {
	...
	turnSpeed = fPController.GetComponent(FPAdventurerInputController).rotationSpeed;
...
function OnGUI () {
   ...
   if(mainMenu) {
      ...
      GUI.Label( Rect (30,0,650,100), introText); // intro text
	  ...
	  if (GUI.Button (Rect (20,270,150,40), "Credits")) {
	...
	if (GUI.Button (buttnRectTemp, "Save Game")) {
	  ...
	  SaveGame(false); // save the current game but don't quit
	}
...
function UpdateControls () { // update GUI from the settings array 
