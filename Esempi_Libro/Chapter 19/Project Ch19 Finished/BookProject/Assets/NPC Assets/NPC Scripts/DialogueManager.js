#pragma strict
var character : GameObject[]; // holds the npc characters
var fpc : GameObject; // need to control user navigation/input during conversation
var gameManager : GameObject; // need to suppress action object text & mouseover
...
topics_1[7] = "007 Can you help me with a replacement?";
...
replies_1[1] = "0001 0 0001^I've been expecting you^You have questions, perhaps?^Hello again^reset";
replies_1[2] = "3030 0 0004 You are in the Maze"; 
replies_1[3] = "0009 0 0005^Trial and error?^Sorry, can't help you with that^It is ever changing, there is no map^No idea, I've been stuck here for ages^I wish I knew";
...
replies_1[7] = "0001 1 0113^Try using this rock^You haven't tried the rock?^Sorry, you're on your own now";
replies_1[8] = "9000 0 0016 Good luck";
...
topics_2[7] = "007 I keep blacking out in the tunnels, any idea where I can find a lantern?";
topics_2[8] = "108 The maze looks different, how do I get out of it?";
topics_2[9] = "109 Thanks for your help";
...
replies_2[2] = "0000 0 0002 He's on break";
...
replies_2[9] = "9009 0 0013^I wish you success^See you around^Good luck^May your quest bear fruit";
...
internal var conversing : boolean = false; // need this to make sure not to interrupt an ongoing conversation
internal var currentGO : GameObject; // the current npc
internal var animator : Animator; // the currentGO's animator component

var topiRock : GameObject;// the rock will need activating
internal var path : GameObject; // the projector light

function Awake () {
   path = GameObject.Find("Path to Tunnels");// find path before it is deactivated
}
...
function Update () {
	...
	if(pauseForReply && Time.time > pauseForReplyLimit) {
	   ...
	   animator.SetBool("Listen", false); // trigger the Listen state/animation off
       animator.SetBool("Talk", true); // trigger the Talk state/animation on
	...
	// stop the talk animation when the audio is finished	
	if(pauseForTopic && Time.time > (pauseForTopicLimit - 0.5)) {
		   print (currentAudioReply.length );	
	   animator.SetBool("Talk", false); // trigger the Talk state/animation off	
	}
	...
	if(pauseForTopic && Time.time > pauseForTopicLimit) {
	   ...
	   if (!exitConv) playerTalking = true;// show the topics if the conversation has not finished
		else { // the conversation is finished
		   conversing = false; // the conversation has finished
		   ...
		   // turn off flag to block picks;
			character[currentNpc].GetComponent(Interactor).processing = false; 
		   // if finished, start the timer to send the npc back to the start position
		   if (character[currentNpc].GetComponent(SimpleAI)) { // if the character the script...
...
function OnGUI () {
   ...
   if (playerTalking) {  // player can select topic
		...
		GUI.BeginGroup (Rect (0, Screen.height - 300 , 900, 600));
		var y = 35; // this will increment the y position for each topic
		
		//text panel
		GUI.Box(Rect (0, 20 , 600, 230),"");	
	
		//Topic 1 
		if (uiText[1] != "") { 
			if (GUI.Button (Rect (0, y, 900, 35), "")) {
			ProcessReply(uiReply[1]); 
			}
			toggleBool = GUI.Toggle (Rect (25, y, 900, 35), false, uiText[1]);
		...
		if (uiText[2] != "") { 
			...
			toggleBool = GUI.Toggle (Rect (25, y, 900, 35), false, uiText[2]);
			y += 35; // increment the next position by 35
		}
		...
		if (uiText[3] != "") { 
			...
			toggleBool = GUI.Toggle (Rect (25, y, 900, 35), false, uiText[3]);
			y += 35; // increment the next position by 35
		}
		...
		if (uiText[4] != "") { 
			...
			toggleBool = GUI.Toggle (Rect (25, y, 900, 35), false, uiText[4]);
			y += 35; // increment the next position by 35
		}
		...
		if (uiText[5] != "") { 
			...
			toggleBool = GUI.Toggle (Rect (25, y, 900, 35), false, uiText[5]);
			y += 35; // increment the next position by 35
		}
	...
	if (npcResponding) {  // npc is talking
	   GUI.Label (Rect (25, 25, 900, 35), textReply);
	}
...
function ProcessReply (replyElement : int) {
	...
	animator.SetBool("Listen", true); // trigger the NPC's Listen state animation on
...
function SetNPC (npcID : int) {
   ...
   switch (npcID) {
	...
	exitConv = false; //reset the flag
	conversing = true; // turn this flag on
...
function SpecialInstructions (code : String) {
  ...
  switch (code) {
    case "01" : // offers rock- this is a one off
	   ...
	   animator.SetBool("Clap", true); // turn the Clap parameter on to trigger its clip
	   yield new WaitForSeconds(0.5); // reset the flag
	   animator.SetBool("Clap", false); // turn the Clap parameter back to false before the animation finishes
...
function SetGO ( npcObj : GameObject) {

   currentGO = npcObj; // the newly active npc -
   animator = currentGO.GetComponent(Animator); // its animator component
}
