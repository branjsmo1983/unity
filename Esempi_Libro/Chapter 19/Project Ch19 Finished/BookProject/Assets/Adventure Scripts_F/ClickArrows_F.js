...
internal var inventoryItems : GameObject;
internal var controlCenter : GameObject;
internal var gridOffset : int; // gridoffset

internal var gridPosition : int;

internal var isActive : boolean = true; // keeps track of state

function Start () {

   inventoryItems = GameObject.Find("Inventory Items");
   controlCenter = GameObject.Find("Control Center2");
   gridOffset = controlCenter.GetComponent(GameManager_F).startPos;
...
function OnMouseExit () {
	...
	guiTexture .color = Color.gray; // return the texture to normal
...
function ShiftGrid (shiftDirection: String) {
	...
	gridPosition = controlCenter.GetComponent(GameManager_F).gridPosition; // get latest gridPosition
	...
	var iLength = controlCenter.GetComponent(GameManager_F).currentInventoryObjects.length;
   ...
   controlCenter.GetComponent(GameManager_F).gridPosition = gridPosition; // send it off 
}


function OnMouseDown () {

	if (!isActive) return; // skip the mouse-down functionality if the arrow is not active
...
function ArrowState (newState : boolean) {
