...
@script RequireComponent(ObjectLookup_F)
...
function Start () {
	...
	controlCenter = GameObject.Find("Control Center2"); // locate and assign the object to the var
...
function Update () {
   ...
   if (pickTimer && Time.time > pickTimerTime -cursorReturn ) { // if time is up and flag is on...
      ...
      controlCenter.GetComponent(GameManager_F).suppressPointer = false;
	}
   ...
   if (pickTimer && Time.time > pickTimerTime) { // if time is up and flag is on...
	  ...
	  controlCenter.GetComponent(GameManager_F).StartWatching ();
	  processing = false;//
   }
...
function OnMouseOver () {
	...
	if (controlCenter.GetComponent(GameManager_F).camMatch) return;
	...
	iMode = controlCenter.GetComponent(GameManager_F).iMode; // get current iMode
	...
	controlCenter.GetComponent(GameManager_F).showText = true;
	//send the correct text to the GameManager for display
	controlCenter.GetComponent(GameManager_F). shortDesc = objectName[currentState];
	controlCenter.GetComponent(GameManager_F).longDesc = description[currentState];

	// automatic bypass flag
	if(!objectIs3D || DistanceFromCamera() <= triggerDistance) {
	        controlCenter.GetComponent(GameManager_F).inRange = true;}
	else  controlCenter.GetComponent(GameManager_F).inRange = false;
...
function OnMouseExit () {
	...
	controlCenter.GetComponent(GameManager_F).showText = false;
...
function OnMouseDown () {
	...
	if (objectIs3D && DistanceFromCamera()  > triggerDistance ) {
	   ...
	   controlCenter.GetComponent(GameManager_F).actionMsg = tempMsg;
	   //tell the GameManager to show the action text
	   controlCenter.GetComponent(GameManager_F).showActionMsg = true;
	   //wait two seconds then turn off the action text and then leave the function
	   yield new WaitForSeconds(2.0);
	   controlCenter.GetComponent(GameManager_F).showActionMsg = false;
	   return;
	}
	...
	cursor = controlCenter.GetComponent(GameManager_F).currentCursor.name;
	if (cursor == controlCenter.GetComponent(GameManager_F).defaultCursor.name) cursor = "default";
	...
	GetComponent(ObjectLookup_F).LookUpState(this.gameObject,currentState,cursor); 
...
function ProcessObject (newState : int) {
   ...
   controlCenter.GetComponent(GameManager_F).suppressPointer = true;
	...
	switch (currentVisibility) {
	  ...
	  case 0 : // deactivate at start 
	      if(currentLocation == 0 ) {
	        controlCenter.GetComponent(GameManager_F).EmergencyTimer(this.name);
...
function Handle2D () {
   ...
   if (previousState == 0 && currentState == 2) {
      controlCenter.GetComponent(GameManager_F).currentCursor = guiTexture.texture;
   ...
   if (previousState == 1 && currentState == 2) {
	...
	if (previousState == 11) { 
	   ...
	   iElement = controlCenter.GetComponent(GameManager_F).replaceElement;
	   yield; // make sure the value has time to update
	   //update the currentInventoryObjects array
	   controlCenter.GetComponent(GameManager_F).OrderInventoryArray(); 
	   //update the inventory grid 
	   controlCenter.GetComponent(GameManager_F).InventoryGrid();
	}
