...
internal var end : boolean; // flag for game end ###
	...
	var cursor : GameObject = gameObject.Find(currentCursor.name);// find the inventory object from its name
		...
		if (didHit) { // you did hit something
		   if (hit.transform.tag == "ActionObject")  return; // do nothing
		      ...
		      cursor.GetComponent(Interactor_F).currentState = 1; // update current state
		      audio.Play();
		   }
		}
...
function OnGUI () {
	...
	if(!end) { // supress icon during end ###
	...
	// cursor control				
	if (!navigating && !suppressPointer && !camMatch && !end) {
...
function AddToInventory (object : GameObject) {
	...
	if(end) return; // game's over
	
	//update the object's element to be the new last element
	object.GetComponent(Interactor_F).iElement = iLength;
...
function RemoveFromInventory (object : GameObject) {
	...
	var iRemove = object.GetComponent(Interactor_F).iElement;
	...
	for (var i : int = 0;i < iObjects.length;i++) {
	   //only if its current state is 1 ( in inventory)
	   if(iObjects[i].GetComponent(Interactor_F).iElement > iRemove) {
	   	  // decrement if the value is greater than the iRemove value
	      iObjects[i].GetComponent(Interactor_F).iElement --; 
	   }
	}
	...
	object.GetComponent(Interactor_F).iElement = 100;
...
function OrderInventoryArray () {
	...
	for (var e : int = 0; e < iLength; e++) { // currentInventoryObjects elements
		for (var i : int = 0; i < iObjects.length; i++) {
			// if there is a match
			if (iObjects[i].GetComponent(Interactor_F).iElement == e) {
...
function NewSettings(playerSettings : String[]) {  // passed in from main menu after gui change

   var fpController = GameObject.Find("First Person Controller");

   //update the the individual vars
   fpController.GetComponent(CharacterMotor).movement.maxForwardSpeed = parseFloat(playerSettings[0]);
   fpController.GetComponent(FPAdventurerInputController_F).rotationSpeed = parseFloat(playerSettings[1]); 
   useText = parseBool(playerSettings[2]); 
   useLongDesc = parseBool(playerSettings[3]);
   // use the index number to get and assign the color swatch's color 
   mouseOverColor = GetComponent(MenuManager_F).cursorColors[parseInt(playerSettings[8])];
}
